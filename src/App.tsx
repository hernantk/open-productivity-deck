...
function Icon({ name }: { name: "volume" | "mute" | "phone" | "plus" | "save" | "refresh" | "folder" | "up" | "down" | "trash" }) {
  const paths = {
...
function ActionPreview({ button, unread }: { button: DeckButton; unread: number | null | undefined }) {
  ...
  return (
    <div className="action-preview" style={{ "--button-color": button.color } as React.CSSProperties}>
      <span className="action-glyph">{initials}</span>
...
function ButtonEditor({ button, index, total, onChange, onMove, onRemove, onPick }: {
  button: DeckButton;
  index: number;
  total: number;
...
export default function App() {
  ...
  function updateVolume(value: number) {
    setDashboard((current) => current ? { ...current, audio: { volume: value / 100, muted: false } } : current);
    if (volumeTimer.current !== null) window.clearTimeout(volumeTimer.current);
  ...
  function addButton() {
    if (!draft) return;
    setDraft({ ...draft, buttons: [...draft.buttons, { id: crypto.randomUUID(), label: "Novo atalho", target: "", kind: "application", color: ACCENTS[draft.buttons.length % ACCENTS.length], unreadProvider: null }] });
  }
    ...
    <main>
      ...
      <section className="hero-grid">
        <div className="audio-panel">
          ...
          <button className={`mute-button ${dashboard?.audio?.muted ? "active" : ""}`} type="button" onClick={() => void toggleMute()}><Icon name={dashboard?.audio?.muted ? "volume" : "mute"} />{dashboard?.audio?.muted ? "Restaurar som" : "Mutar saída"}</button>
        </div>
